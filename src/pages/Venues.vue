<template>
  <div class="h-[calc(100vh-4rem)] flex flex-col">
    ...
    <div class="flex-1 flex overflow-hidden">
      <!-- Venues List -->
      <div :class="['overflow-y-auto bg-gray-50', showMap ? 'hidden md:block w-full md:w-1/2 lg:w-2/5' : 'block w-full']">
        <div class="p-4 space-y-4">
          <div class="flex items-center justify-between mb-4">
            <div>
              ...
              <p v-if="!user" class="text-sm text-gray-600 mt-1">
                Showing first 3 venues. <router-link to="/auth/signup" class="text-emerald-600 hover:text-emerald-700 font-medium">Sign up</router-link> to view all.
              </p>
            </div>
            <button @click="showMap = !showMap" class="md:hidden px-4 py-2 bg-emerald-600 text-white rounded-lg">
          ...
          <div
            ...
            :class="[
              ...
              isBlurred(venue, index) ? 'cursor-default' : 'cursor-pointer hover:shadow-xl hover:scale-[1.02]'
            ]"
          >
            <!-- Blur Overlay -->
            <div v-if="isBlurred(venue, index)" class="absolute inset-0 bg-white bg-opacity-80 backdrop-blur-md rounded-xl z-10 flex items-center justify-center">
              <div class="text-center">
                <div class="text-4xl mb-4">Lock</div>
                <h3 class="text-xl font-bold mb-2">Sign Up to See More</h3>
                <router-link to="/auth/signup" class="px-6 py-3 bg-emerald-600 text-white rounded-lg font-bold">
                  Sign Up Free
                </router-link>
              </div>
            </div>
            ...
            <div :class="{ 'filter blur-sm': isBlurred(venue, index) }">
              <div class="flex gap-4">
                <!-- Gradient Icon -->
                <div class="w-32 h-32 rounded-lg overflow-hidden bg-gradient-to-br flex items-center justify-center text-5xl shadow-lg"
                ...
                <!-- Info -->
                <div class="flex-1">
                  <h3 class="text-xl font-bold truncate" style="font-family: var(--font-display);">
                    {{ venue.name }}
                  </h3>
                  ...
                  <div class="flex items-center gap-3 mt-3">
                    <span class="px-3 py-1 rounded-full text-xs font-bold text-white"
                          :style="{ backgroundColor: venue.categories?.color || '#10b981' }">
                      {{ venue.categories?.name || 'Unknown' }}
                    </span>
                    <span v-if="venue.price" class="text-gray-600 font-medium">{{ venue.price }}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <!-- Map -->
      <div :class="['bg-gray-200', showMap ? 'block w-full md:w-1/2 lg:w-3/5' : 'hidden md:block']">
...
const filteredVenues = computed(() => {
  ...
  if (selectedCategories.value.length > 0 && !selectedCategories.value.includes('all')) {
    list = list.filter(v => selectedCategories.value.includes(v.categories?.name))
  }
